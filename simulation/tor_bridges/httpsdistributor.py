...
class HTTPSDistributor(Distributor):
    ...
    :type proxies: :class:`~proxies.ProxySet`
    :ivar proxies: All known proxies, which we treat differently. See
        :param:`proxies`.
    :type hashring: :class:`capitalbridges.FilteredBridgeSplitter`
    ...
    def __init__(self, totalSubrings, key, proxies=None, answerParameters=None):
        ...
        :type answerParameters: :class:`capitalbridges.BridgeRingParameters`
        ...
        if proxies:
            print("Added known proxies to HTTPS distributor...")
            self.proxies = proxies
            self.totalSubrings = self.totalSubrings + 1
            self.proxySubring = self.totalSubrings
        else:
            print("No known proxies were added to HTTPS distributor!")
            self.proxies = proxy.ProxySet()
            self.proxySubring = 0

        self.ringCacheSize = self.totalSubrings * 3

        key2 = getHMAC(key, b"Assign-Bridges-To-Rings")
        key3 = getHMAC(key, b"Order-Areas-In-Rings")
        key4 = getHMAC(key, b"Assign-Areas-To-Rings")
        ...
        self.name = 'HTTPS'
        print("Added %s to %s distributor." %
    ...
    def getSubnet(cls, ip, usingProxy=False, proxySubnets=4):
        ...
        print("Client IP was within area: %s" % subnet)
        return subnet
    ...
    def mapClientToHashringPosition(self, interval, subnet):
        ...
        **interval** which the client's request occurred within, as well as
        ...
        mapping = self._clientToPositionHMAC(bytes(position,'utf-8'))
        return mapping
    ...
    def prepopulateRings(self):
        ...
        for filterFn in [byIPv4, byIPv6]:
            for subring in range(1, self.totalSubrings + 1):
                ...
                key1 = getHMAC(self.key, b"Order-Bridges-In-Ring-%d" % subring)
    ...
    def _buildHashringFilters(self, previousFilters, subring):
        print("self.hashring.hmac")
        print(type(self.hashring.hmac))
        print(self.hashring.hmac)
    ...
    def getBridges(self, bridgeRequest, interval):
        ...
        :rtype: list
        :return: A list of :class:`~capitalbridges.Bridge`s to include in
            the response. See
        ...
        if not len(self.hashring):
            print("Bailing! Hashring has zero bridges!")
            return []
        ...
        if bridgeRequest.client in self.proxies:
            ...
            print("Client was from known proxy (tag: %s): %s" %
                         (tag, bridgeRequest.client))
        print(bridgeRequest.client)
        ...
        print("Client request within time interval: %s" % interval)
        print("Assigned client to subhashring %d/%d" % (subring, self.totalSubrings))
        print("Assigned client to subhashring position: %s" % str(position))

        #print("Assigned client to subhashring position: %s" % position.encode('hex'))
        print("Total bridges: %d" % len(self.hashring))
        #print("Bridge filters: %s" % ' '.join([x.func_name for x in filters]))

        # Check wheth we have a cached copy of the hashring:
        if filters in self.hashring.filterRings.keys():
            print("Cache hit %s" % filters)
            _, ring = self.hashring.filterRings[filters]
        # Otherwise, construct a new hashring and populate it:
        else:
            print("Cache miss %s" % filters)
