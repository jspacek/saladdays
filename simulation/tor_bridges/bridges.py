# -*- coding: utf-8 ; test-case-name: test.test_bridges -*-
#
...
from crypto import removePKCS1Padding
from parse.addr import isIPAddress
from parse.addr import isIPv4
from parse.addr import isIPv6
from parse.addr import isValidIP
from parse.addr import PortList
from parse.fingerprint import isValidFingerprint
from parse.fingerprint import toHex
from parse.fingerprint import fromHex
from parse.nickname import isValidRouterNickname
from util import isascii_noncontrol
...
class BridgeAddressBase(object):
    ...
    def fingerprint(self, value):
        ...
        .. info: The purported fingerprint will be checked for specification
            conformity with
            :func:`~parse.fingerprint.isValidFingerprint`.
...
class BridgeBase(BridgeAddressBase):
    ...
    def nickname(self, value):
        ...
        .. note:: We don't need to call
            :func:`parse.nickname.isValidRouterNickname() since Stem
...
class BridgeBackwardsCompatibility(BridgeBase):
    ...
    def __init__(self, nickname=None, ip=None, orport=None,
        ...
        """Create a :class:`Bridge <bridges.IBridge>` which is
    ...
    def _backwardsCompatible(self, nickname=None, address=None, orPort=None,
        ...
        version of this class (see :class:`test.deprecated.Bridge`).
        """
        ...
        if orAddresses and isinstance(orAddresses, dict):
            for ip, portlist in orAddresses.items():
                ...
                if validAddress:
                    # The old code expected a `parse.addr.PortList`:
    ...
    def getConfigLine(self, includeFingerprint=False, addressClass=None,
        ...
        The old ``Bridges.Bridge.getConfigLine()`` method didn't know
        about :class:`~bridgerequest.BridgeRequestBase`s, and so this
        modified version is backwards compatible by creating a
        :class:`~bridgerequest.BridgeRequestBase` for
        :meth:`getBridgeLine`. The default parameters are the same as they
        were in the old ``Bridges.Bridge`` class.
    ...
    def familiar(self):
        ...
        eight days.
        """
        with Storage.getDB() as db:  # pragma: no cover
...
class Bridge(BridgeBackwardsCompatibility):
    ...
    :vartype flags: :class:`~bridges.Flags`
    ...
    def _getTransportForRequest(self, bridgeRequest):
        ...
        **bridgeRequest**.
        ...
        :type bridgeRequest: :class:`bridgerequest.BridgeRequestBase`
    ...
    def assertOK(self):
        ...
        1. Any IP addresses contained in :data:`orAddresses` are valid,
           according to :func:`~parse.addr.isValidIP`.
        ...
        .. todo:: This should probably be reimplemented as a property that
            ...
            for :data:`bridges.BridgeAddressBase.address` and
            :data:`bridges.BridgeBase.orPort`.
    ...
    def updateFromServerDescriptor(self, descriptor, ignoreNetworkstatus=False):
        ...
        .. note::
            If :func:`~parse.descriptor.parseServerDescriptorFile` is
