...
def client_exposure_boxplot():
    """
     Track the total of honest clients explosed over time comparing 3 algorithms in a boxplot
    """
    print("________________________________Client Exposure____________________________________\n")
    ...
    for i in range(0, util.NUM_TRIALS):
        ...
        for i in range(0, len(df_all)):
            ...
            #print(files[i])
            # a future client exposure from previously exposed proxy and client exposures from prior assignments
            df = df[(df.action == 'EXPOSE_CLIENT') | (df.action == 'ENUMERATE_PROXY')].round()
    ...
    for df, group in zip(data, data_groups):
        for name, bin in df:
            max_list = bin.groupby(['trial'], as_index=False, sort=False)['honest_clients'].max()
            max = max_list['honest_clients'].tolist()
            group.append(max)

    # Create graph with all trials and experiments
    title="Number of honest clients exposed over time in %d trials using %d proxies" % (util.NUM_TRIALS, util.NUM_PROXIES)
    ...
    fig.text(0.04, 0.5, 'Number of Total Exposed Honest Clients', va='center', rotation='vertical')
    ...
    #plt.show()
    graphnamepng = "analysis/archive/client_boxplot_%d_trials_%d_proxies_%d_victim_set.png" % (util.NUM_TRIALS, util.NUM_PROXIES, util.VICTIM_SET)
    plt.savefig(graphnamepng)


if __name__ == '__main__':
    client_exposure_boxplot()
